		...
		try {
			...
			if (result > 0) {
				...
				RDFXMLDocumentFormat rdfDocFormat = ("true".equalsIgnoreCase(properties.getProperty("expandOntology")))
						...
						RDFConnection conn = RDFConnection.connectPW(local,"FusekiUser","passwordUser");

						conn.put("URL_Fuseki" + narrative.getAuthor() + "_" + narrative.getId(), fileURI.toString().split(":")[1]);
						conn.put("URL_Fuseki" + narrative.getAuthor() + "_" + narrative.getId(), "./Target/n.owl");
